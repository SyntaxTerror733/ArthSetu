 ...
 */
export const getCategoryInsights = (cat) => {
  switch (cat) {
    case 'Dairy':
      return {
...
export const renderSwotBlock = (realContent, defaultList, icon) => {
...
export const renderNarrative = (realVal, defaultListOrRender) => {
  if (realVal) {
...
/**
 * Reusable Section Card Components for Compare mode & Single Report mode
 */

export function OptionHeaderCard({ optionLabel, category, verdict, currentLang }) {
  const isWinner = verdict?.recommended_category === category;
  return (
    <div
      style={{
        backgroundColor: isWinner ? '#ECFDF5' : '#F8FAFC',
        borderTop: isWinner ? '4px solid #059669' : '1px solid #CBD5E1',
...
export function MarketReachCard({ location, category, reportData, insights, currentLang }) {
  return (
    <div className="dashboard-card" style={{ height: '100%', display: 'flex', flexDirection: 'column', boxSizing: 'border-box' }}>
      ...
      {reportData?.market_reach ? (
        ...
        <div className="market-reach-stats" style={{ flex: 1 }}>
...
export function OpportunityAnalysisCard({ location, category, reportData, insights, currentLang }) {
  return (
    <div className="dashboard-card" style={{ height: '100%', display: 'flex', flexDirection: 'column', boxSizing: 'border-box' }}>
      <div className="card-title-row">
        <h3 className="card-heading">
...
export function SwotQuadrantCard({ type, reportData, insights, currentLang }) {
  let title = '';
  let badge = null;
  let icon = null;
  ...
  if (type === 'strengths') {
    title = currentLang === 'hi' ? 'ताकत (Strengths)' : 'Strengths';
    badge = <span className="badge badge-emerald">S</span>;
    icon = <CheckCircle2 size={16} className="text-emerald swot-bullet-icon" />;
    realContent = reportData?.swot?.strengths;
    defaultList = insights.swot.strengths;
    cellClass = 'swot-cell strengths';
  } else if (type === 'weaknesses') {
    title = currentLang === 'hi' ? 'कमजोरियां (Weaknesses)' : 'Weaknesses';
    badge = <span className="badge badge-saffron">W</span>;
    icon = <AlertCircle size={16} className="text-saffron swot-bullet-icon" />;
    realContent = reportData?.swot?.weaknesses;
    defaultList = insights.swot.weaknesses;
    cellClass = 'swot-cell weaknesses';
  } else if (type === 'opportunities') {
    title = currentLang === 'hi' ? 'अवसर (Opportunities)' : 'Opportunities';
    badge = <span className="badge badge-teal">O</span>;
    icon = <TrendingUp size={16} className="text-teal swot-bullet-icon" />;
    realContent = reportData?.swot?.opportunities;
    defaultList = insights.swot.opportunities;
    cellClass = 'swot-cell opportunities';
  } else if (type === 'threats') {
    title = currentLang === 'hi' ? 'चुनौतियां (Threats)' : 'Threats';
    badge = (
      <span
        style={{
    ...
    icon = <AlertTriangle size={16} style={{ color: '#E11D48' }} className="swot-bullet-icon" />;
    realContent = reportData?.swot?.threats;
    defaultList = insights.swot.threats;
    cellClass = 'swot-cell threats';
  }

  return (
    <div className={cellClass} style={{ height: '100%', boxSizing: 'border-box', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ flex: 1 }}>
        {renderSwotBlock(realContent, defaultList, icon)}
      </div>
    </div>
  );
}

export function LocalThreatsCard({ reportData, insights, currentLang }) {
  return (
    <div className="dashboard-card" style={{ height: '100%', display: 'flex', flexDirection: 'column', boxSizing: 'border-box' }}>
      <div className="card-title-row">
        <h3 className="card-heading">
      ...
      <div className="threats-list" style={{ flex: 1 }}>
...
export function CompetitorMappingCard({ reportData, insights, currentLang }) {
  return (
    <div className="dashboard-card" style={{ height: '100%', display: 'flex', flexDirection: 'column', boxSizing: 'border-box' }}>
      <div className="card-title-row">
        <h3 className="card-heading">
...
export function PricingSuggestionCard({ reportData, insights, currentLang }) {
  return (
    <div className="dashboard-card" style={{ height: '100%', display: 'flex', flexDirection: 'column', boxSizing: 'border-box' }}>
      <div className="card-title-row">
        <h3 className="card-heading">
      ...
      {reportData?.pricing_suggestion ? (
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
          ...
          <div>
            <div className="price-benchmark-box" style={{ marginBottom: '1rem' }}>
...
/**
 * Module 1: Business Feasibility Report (Single-district full report view for Result.jsx)
 */
export default function FeasibilityReport({
  ...
  // 2. Error State View for Module 1
  if (error) {
    return (
      <section className="dashboard-module" id="module-feasibility">
        <div
    ...
    <section className="dashboard-module" id={id}>
      ...
      <div className="dashboard-card" style={{ marginBottom: '1.5rem' }}>
        ...
        <div className="swot-2x2-grid">
          <SwotQuadrantCard type="strengths" reportData={reportData} insights={insights} currentLang={currentLang} />
          <SwotQuadrantCard type="weaknesses" reportData={reportData} insights={insights} currentLang={currentLang} />
          <SwotQuadrantCard type="opportunities" reportData={reportData} insights={insights} currentLang={currentLang} />
          <SwotQuadrantCard type="threats" reportData={reportData} insights={insights} currentLang={currentLang} />
        </div>
      </div>

      {/* Grid Row 3: 4. Local Threats, 5. Competitor Mapping & 6. Product Market Value */}
      <div className="grid-3">
        <LocalThreatsCard reportData={reportData} insights={insights} currentLang={currentLang} />
        <CompetitorMappingCard reportData={reportData} insights={insights} currentLang={currentLang} />
        <PricingSuggestionCard reportData={reportData} insights={insights} currentLang={currentLang} />
      </div>
